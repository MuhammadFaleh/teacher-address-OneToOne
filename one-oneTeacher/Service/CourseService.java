...
import com.JPARelation.one_oneTeacher.DTO.OUT.*;
...
public class CourseService{
    ...
    public Course convertDtoToCourse(CourseDTOIN courseDTOIN){
        ...
        if(teacher == null){
            return new Course(courseDTOIN.getCourse_id(), courseDTOIN.getName(), null, null);
        }
        return new Course(courseDTOIN.getCourse_id(),courseDTOIN.getName(), teacher, null);
    }
    ...
    public CourseStudentDTOOUT getCourseStudents(Integer course_id){
        Course course = courseRepository.findCourseById(course_id);
        Set<StudentDTOOUT> studentDTOOUTS = new HashSet<>();
        if(course == null){
            throw  new ApiException("course not found");
        }
        if(!course.getStudents().isEmpty() && course.getTeacher() != null){
            for (Student student : course.getStudents()){
                studentDTOOUTS.add(new StudentDTOOUT(student.getId(),student.getName(),student.getAge(),student.getMajor()));
            }
            return new CourseStudentDTOOUT(course.getId(),course.getTeacher().getName(),course.getTeacher().getEmail(),studentDTOOUTS);
        }
        return new CourseStudentDTOOUT(course.getId(), null,null,null);
    }
